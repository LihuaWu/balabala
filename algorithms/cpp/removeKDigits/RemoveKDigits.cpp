    ...
    string removeKdigits_pick(string& num, int k) {
        ...
        for (int i = 0; i < len - k; i++) {
            ...
            if (!(result.empty() && num[min_idx] == '0')) {
                cout << num[min_idx] << " min_idx:" << min_idx << "\n";
                result.push_back(num[min_idx]);
            }

            // select the number started from next one, to make the order
            // correctness.
            idx = min_idx + 1;
        }
    ...
    string removeKdigits_remove(string& num, int k) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            for (int j = 0; j < num.size() - 1; j++) {
                // if the current is bigger than next one, then revmoe the
                // current one. In other word, we always pick the smaller one
                // number.
                if (num[j] > num[j + 1]) {
                    num.erase(j, 1);
                    idx = j;
...
int main() {
    Solution sl;
    string s("10010");
    cout << sl.removeKdigits_pick(s, 1) << "\n";
